...
void CryptoBot::displayTransactions() {
    ...
    for (const auto& transaction : transactions) {
        ...
        std::string coinId = transaction["coin_id"];
        double price = transaction["price"];
        double quantity = transaction["quantity"];
        double profit = transaction["profit"];
        std::time_t timestamp = transaction["timestamp"];

        // Convert timestamp to readable format using localtime_s
        std::tm timeInfo;
        localtime_s(&timeInfo, &timestamp);
        char timeBuffer[80];
        strftime(timeBuffer, sizeof(timeBuffer), "%Y-%m-%d %H:%M:%S", &timeInfo);

        std::cout << "Coin: " << coinId << "\n";
        std::cout << "Type: " << type << "\n";
        std::cout << "Price: " << price << "\n";
        std::cout << "Quantity: " << quantity << "\n";
        std::cout << "Profit: " << profit << "\n";
        std::cout << "Timestamp: " << timeBuffer << "\n";
        std::cout << "-----------------------------\n";
    }
}
